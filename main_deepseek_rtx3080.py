...
def main():
    parser = argparse.ArgumentParser(
        description="DeepSeek OCR 기반 문서 OCR (RTX 3080 최적화)"
    )
    parser.add_argument(
        "file",
    ...
    parser.add_argument(
        "-o", "--output",
        default=None,
        help="저장할 텍스트 파일 경로 (미지정 시 입력파일명_deepseek_rtx3080.txt)",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    try:
        logger.info("=" * 60)
        logger.info("DeepSeek OCR 시작 (RTX 3080 최적화)")
        logger.info(f"입력 파일: {args.file}")
        logger.info(f"특징: OCR 전용, 가장 빠른 속도")
        logger.info("=" * 60)
