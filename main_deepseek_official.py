...
from loader_deepseek_official import load_document
...
def main():
    parser = argparse.ArgumentParser(
        description="DeepSeek OCR 문서 OCR (공식 구현)"
    )
    parser.add_argument(
        "file",
    ...
    parser.add_argument(
        "-o", "--output",
        default=None,
        help="저장할 텍스트 파일 경로 (미지정 시 입력파일명_deepseek_official.txt)",
    )
    parser.add_argument(
        "-m", "--mode",
        default="gundam",
    ...
    try:
        logger.info("=" * 60)
        logger.info("DeepSeek OCR 시작 (공식 구현)")
        ...
        if args.output:
            ...
            p = Path(args.file)
            out_path = p.parent / f"{p.stem}_deepseek_official.txt"
