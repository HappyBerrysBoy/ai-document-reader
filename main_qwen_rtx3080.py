...
from loader_qwen import load_document, summarize_text
...
def main():
    parser = argparse.ArgumentParser(
        description="Qwen3-VL 기반 문서 OCR (RTX 3080 최적화, 한글/영어)"
    )
    parser.add_argument(
        "file",
    ...
    parser.add_argument(
        "-o", "--output",
        default=None,
        help="저장할 텍스트 파일 경로 (미지정 시 입력파일명_qwen_rtx3080.txt)",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    try:
        logger.info("=" * 60)
        logger.info("Qwen3-VL OCR 시작 (RTX 3080 최적화)")
        logger.info(f"입력 파일: {args.file}")
        logger.info(f"모델: Qwen3-VL-4B (빠른 속도)")
        logger.info("=" * 60)
        ...
        if args.output:
            ...
            p = Path(args.file)
            out_path = p.parent / f"{p.stem}_qwen_rtx3080.txt"
